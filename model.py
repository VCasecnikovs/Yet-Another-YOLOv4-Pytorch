...
class ECA(nn.Module):
    ...
    def forward(self, x):
        ...
        channel_features = (
            self.conv(squized_channels.squeeze(-1).transpose(-1, -2))
            .transpose(-1, -2)
            .unsqueeze(-1)
        )
...
class Conv2dWS(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        std = (
            torch.sqrt(torch.var(weight.view(weight.size(0), -1), dim=1) + 2e-5).view(
                -1, 1, 1, 1
            )
            + 1e-5
        )
...
class AddCoordChannels(nn.Module):
    def __init__(self, w=9, h=9, b=1):
        ...
        self.b = b
        self.y_coords = (
            2.0 * torch.arange(h).unsqueeze(1).expand(h, w) / (h - 1.0) - 1.0
        )
        self.x_coords = (
            2.0 * torch.arange(w).unsqueeze(0).expand(h, w) / (w - 1.0) - 1.0
        )
    ...
    def __init__(
        ...
        if bn:
            if bcn:
                ...
                modules.append(
                    nn.BatchNorm2d(out_channels, track_running_stats=not ws)
                )  # IF WE ARE NOT USING track running stats and using WS, it just explodes.
        if activation == "mish":
            if hard_mish:
            ...
            raise BadArguments(
                "Please use one of suggested activations: mish, relu, leaky, linear."
            )
...
class PAN_Layer(nn.Module):
    def __init__(
        ...
        # Gets input from d4
        self.c2_from_upsampled = ConvBlock(
            in_c,
            out_c,
            1,
            1,
...
class ASFF(nn.Module):
    def __init__(self, level, rfb=False, vis=False, bcn=False, mbn=False):
        ...
        if level == 0:
            self.stride_level_1 = ConvBlock(
                256, self.inter_dim, 3, 2, "leaky", bcn=bcn, mbn=mbn
            )
            self.stride_level_2 = ConvBlock(
                128, self.inter_dim, 3, 2, "leaky", bcn=bcn, mbn=mbn
            )
            self.expand = ConvBlock(
                self.inter_dim, 512, 3, 1, "leaky", bcn=bcn, mbn=mbn
            )
        elif level == 1:
            self.compress_level_0 = ConvBlock(
                512, self.inter_dim, 1, 1, "leaky", bcn=bcn, mbn=mbn
            )
            self.stride_level_2 = ConvBlock(
                128, self.inter_dim, 3, 2, "leaky", bcn=bcn, mbn=mbn
            )
            self.expand = ConvBlock(
                self.inter_dim, 256, 3, 1, "leaky", bcn=bcn, mbn=mbn
            )
        elif level == 2:
            self.compress_level_0 = ConvBlock(
                512, self.inter_dim, 1, 1, "leaky", bcn=bcn, mbn=mbn
            )
            self.compress_level_1 = ConvBlock(
                256, self.inter_dim, 1, 1, "leaky", bcn=bcn, mbn=mbn
            )
            self.expand = ConvBlock(
                self.inter_dim, 128, 3, 1, "leaky", bcn=bcn, mbn=mbn
            )

        compress_c = (
            8 if rfb else 16
        )  # when adding rfb, we use half number of channels to save memory
        self.weight_level_0 = ConvBlock(
            self.inter_dim, compress_c, 1, 1, "leaky", bcn=bcn, mbn=mbn
        )
        self.weight_level_1 = ConvBlock(
            self.inter_dim, compress_c, 1, 1, "leaky", bcn=bcn, mbn=mbn
        )
        self.weight_level_2 = ConvBlock(
            self.inter_dim, compress_c, 1, 1, "leaky", bcn=bcn, mbn=mbn
        )
...
class ACFF(nn.Module):
    def __init__(self, level, rfb=False, vis=False, bcn=False, mbn=False):
        ...
        self.weights_spatial = torch.nn.init.kaiming_uniform_(
            self.weights_spatial, a=0, mode="fan_in", nonlinearity="relu"
        )
    ...
    def forward(self, x_level_0, x_level_1, x_level_2):
        ...
        levels_weight = torch.cat(
            [level_0_flattened, level_1_flattened, level_2_flattened], dim=2
        ).squeeze(-1)

        # For each channel, getting 3 values, they would show how much attention should we give for each level's channel
        level_weight = torch.einsum(
            "bci, kci-> bck", levels_weight, self.weights_spatial
        )
...
class Neck(nn.Module):
    def __init__(
        ...
        # SPP block
        self.mp4_1 = nn.MaxPool2d(
            kernel_size=spp_kernels[0], stride=1, padding=spp_kernels[0] // 2
        )
        self.mp4_2 = nn.MaxPool2d(
            kernel_size=spp_kernels[1], stride=1, padding=spp_kernels[1] // 2
        )
        self.mp4_3 = nn.MaxPool2d(
            kernel_size=spp_kernels[2], stride=1, padding=spp_kernels[2] // 2
        )
        ...
        if (
            asff
        ):  # branch inputs biggest objects: 512, medium objects: 256, smallest objects : 128
            self.ASFF_0 = ASFF(0)
            self.ASFF_1 = ASFF(1)
            self.ASFF_2 = ASFF(2)
...
class YOLOLayer(nn.Module):
    ...
    def calculate_repullsion(self, y, y_hat):
        ...
        for bn in range(batch_size):
            ...
            different_target_mask = ind[have_target_mask, 0] != ind[
                have_target_mask, 0
            ].unsqueeze(1)
    ...
    def forward(self, x: torch.Tensor, targets=None):
        ...
        prediction = (
            x.view(
                num_samples,
                self.num_anchors,
                self.num_classes + not_class_channels,
                grid_size,
                grid_size,
            )
            .permute(0, 1, 3, 4, 2)
        ...
        v = (4 / (math.pi ** 2)) * torch.pow(
            (
                torch.atan(tw[obj_mask] / th[obj_mask])
                - torch.atan(w[obj_mask] / h[obj_mask])
            ),
            2,
        )
        ...
        print(f"CIoUloss:{CIoUloss} | loss_cls:{loss_cls} | loss_conf:{loss_conf}")
        print(f"iou.max(): {iou.max()} | pred_conf.max(): {pred_conf.max()}")
        print(f"w.max():{w.max()} | h.max():{h.max()}")
        ...
        # print(f"C: {c}; D: {d}")
        # print(
        #     f"Confidence is object: {loss_conf_obj}, Confidence no object: {loss_conf_noobj}"
        # )
        # print(f"IoU: {iou_masked}; DIoU: {rDIoU}; alpha: {alpha}; v: {v}")
        # print(
        #     f"CIoU : {CIoUloss.item()}; Confindence: {loss_conf.item()}; Class loss should be because of label smoothing: {loss_cls.item()}"
        # )
            ...
            try:  # If we change input or output layers amount, we will have an option to use pretrained weights
                self.load_state_dict(
                    torch.hub.load_state_dict_from_url(
                        "https://github.com/VCasecnikovs/Yet-Another-YOLOv4-Pytorch/releases/download/V1.0/yolov4.pth"
                    ),
                    strict=False,
                )
            except RuntimeError as e:
